...
class Masking():
    ...
    def __init__(
        ...
        self.len_all_chan_groups = len(params.CHANNEL_GROUPS)+1 # dynamic world included
    ...
    def random_masking(
        ...
        random_srtm =random.random()<=self.ratio # select value smaller equals ratio (0.5)
        if random_srtm:
        ...
        chosen_indices = valid_indices[np.random.choice(len(valid_indices), num_masked_tokens, replace=False)]
    ...
    def channel_groups_masking (
        ...
        chan_idx=[value['id'] for chan, value in params.CHANNEL_GROUPS.items() if (chan != 'SRTM')]
        chan_idx.append('DW')
        ...
        # mask the rest
        return self.random_masking(eo_mask, dw_mask, srtm_mask, num_masked_tokens, srtm_id)
    ...
    def cont_timesteps_masking (
        ...
        masked_month_idx=[(start_month+i)%(self.time_steps) for i in range(num_masked_steps)]
    ...
    def create_mask(self, strategy):
        ...
        srtm_mask = False
        num_masked_tokens = int((self.time_steps*self.len_all_chan_groups)*self.ratio) # include dynamic world here 
        srtm_id = int(params.CHANNEL_GROUPS['SRTM']['id']) # exclude SRTM by setting values True

        # MASKING_STRATEGIES=('random','channel_groups', 'contiguous_timesteps', 'timesteps')
        if strategy=='random':
            eo_mask, dw_mask, srtm_mask = self.random_masking(eo_mask, dw_mask, srtm_mask, num_masked_tokens, srtm_id)

        # mask entire channel groups at once
        elif strategy=='channel_groups':
            eo_mask, dw_mask, srtm_mask = self.channel_groups_masking(eo_mask, dw_mask, srtm_mask, num_masked_tokens, srtm_id)

        # masks continuous timestep across all channels
        elif strategy=='contiguous_timesteps':
            eo_mask, dw_mask, srtm_mask = self.cont_timesteps_masking(eo_mask, dw_mask, srtm_mask, num_masked_tokens, srtm_id)

        elif strategy=='timesteps':
            eo_mask, dw_mask, srtm_mask = self.timesteps_masking(eo_mask, dw_mask, srtm_mask, num_masked_tokens, srtm_id)
        ...
        eo_mask = np.repeat(
            ...
            axis=1) # repeat each mask for the number of encoded channels (columns) per channel group
    ...
    def mask_input(self, eo_data, dw_data):
        ...
        for strategy in self.strategies:
            assert strategy in params.MASKING_STRATEGIES, f'Masking strategy {strategy} not implemented.'
        ...
        masked_dw_tokens = np.ones_like(dw_data) * params.DW_CLASSES # label masked data as 9, labels 0-8 for dynamic world normal data
