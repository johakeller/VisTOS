...
class MultiTempCropClass(IterableDataset):
    ...
    def __iter__(self):
        ...
        for idx, sample_id in enumerate(self.sample_idx):
            ...
            eo_style_array = torch.zeros(
                [
                    self.num_bands,
                    params.MTCC_MAX_SEQ_LEN,
                    params.MTCC_IMG_WIDTH,
                    params.MTCC_IMG_WIDTH,
                ]
            )
            ...
            eo_style_array, eo_style_mask, eo_style_label, coords, start_month = (
                self.prepare_channel_input(sample_id, eo_style_array, eo_style_mask)
            )
            ...
            for pix_id in range(params.MTCC_NUM_PIXELS):
                ...
                if idx == self.num_samples and pix_id >= (
                    self.data_length % params.MTCC_NUM_PIXELS
                ):
                    break
                ...
                dw_data_vf = torch.zeros(eo_mask_vf.shape[0], dtype=torch.float32).to(
                    params.DEVICE
                )
    ...
    def get_month_idx(self, sample_id):
        ...
        start_month = (
            pd.to_datetime(sample_meta["first_img_date"]).to_period("M")
        ).month - 1
        # get middle month index
        mid_month = (
            pd.to_datetime(sample_meta["middle_img_date"]).to_period("M")
        ).month - 1
        # get last month index
        last_month = (
            pd.to_datetime(sample_meta["last_img_date"]).to_period("M")
        ).month - 1
    ...
    def prepare_channel_input(self, sample_id, eo_tensor, eo_mask):
        ...
        eo_data = eo_data.reshape(
            params.MTCC_TIME_STEPS,
            params.MTCC_NUMBER_CHANNELS,
            params.MTCC_IMG_WIDTH,
            params.MTCC_IMG_WIDTH,
        )
