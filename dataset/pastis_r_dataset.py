...
class PastisRDataset(IterableDataset):
    ...
    def __init__(
        ...
        if max_length is not None:
            if max_length > self.data_length:
                raise IndexError(
                    f"Length {max_length} is exceeding actual number of samples ({self.data_length})"
                )
            else:
        ...
        for s in self.sats:
            with open(
                os.path.join(params.P_PATH, "NORM_{}_patch.json".format(s)), "r"
            ) as file:
    ...
    def __iter__(self):
        ...
        for idx, item in enumerate(self.patch_ids):
            ...
            num_orig_bands = (
                sum(values["length"] for keys, values in params.CHANNEL_GROUPS.items())
                + 1
            )  # include DW, include B9
            ...
            eo_style_array, eo_style_mask, eo_style_label, coords = (
                self.prepare_channel_input(item, eo_style_array, eo_style_mask)
            )
            ...
            for pix_id in range(params.P_NUM_PIXELS):
                ...
                eo_label = torch.tensor(
                    eo_style_label[0][pix_id], device=params.DEVICE, dtype=torch.long
                )
                dw_data = torch.zeros(eo_mask.shape[0], dtype=torch.float32).to(
                    params.DEVICE
                )
    ...
    def calculate_coords(coordinates):
        ...
        transform = from_bounds(
            x_left, y_bottom, x_right, y_top, params.P_IMG_WIDTH, params.P_IMG_WIDTH
        )

        # target matrix
        coord_matrix = np.zeros(
            (2, params.P_IMG_WIDTH, params.P_IMG_WIDTH), dtype=np.float32
        )
    ...
    def calc_monthly_median(self, id_patch, id_s, band_data):
        """
        ...
        # get the real date
        day_table = [
            self.reference_date + timedelta(days=int(day)) for day in valid_days
        ]
    ...
    def normalize(self, band_data, s_id, plottable_norm=True):
        """
        ...
        band_data_std_norm = (
            band_data - self.norm[s_id][0][None, :, None, None]
        ) / self.norm[s_id][1][None, :, None, None]
        ...
        if plottable_norm:
            ...
            for chan_id in range(band_data.shape[0]):
                ...
                if chan_min < chan_max:
                    band_data_plot_norm[chan_id] = (channel - chan_min) / (
                        chan_max - chan_min
                    )
            return band_data_plot_norm
        return band_data_std_norm

    def prepare_channel_input(self, id_patch, eo_style_array, eo_style_mask):
        """
        ...
        s1a_data = np.load(
            os.path.join(params.P_S1A_PATH, f"S1A_{id_patch}.npy")
        )  # use only ascending
        # Sentinel 2 (t, c, h, w)
        s2_data = np.load(os.path.join(params.P_S2_PATH, f"S2_{id_patch}.npy"))
        # patch label (3, h, w)
        region_label = np.load(
            os.path.join(params.P_PATH, "ANNOTATIONS", f"TARGET_{id_patch}.npy")
        )
        ...
        eo_style_array[
            list(params.CHANNEL_GROUPS["S2_RGB"]["idx"]), : params.P_MAX_SEQ_LEN
        ] = s2_data_norm[:3]
        # update EO mask
        eo_style_mask[
            list(params.CHANNEL_GROUPS["S2_RGB"]["idx"]), : params.P_MAX_SEQ_LEN
        ...
        eo_style_array[
            list(params.CHANNEL_GROUPS["S2_Red_Edge"]["idx"]), : params.P_MAX_SEQ_LEN
        ] = s2_data_norm[3:6]
        eo_style_mask[
            list(params.CHANNEL_GROUPS["S2_Red_Edge"]["idx"]), : params.P_MAX_SEQ_LEN
        ] = 0
        # insert NIR 10
        eo_style_array[
            list(params.CHANNEL_GROUPS["S2_NIR_10"]["idx"]), : params.P_MAX_SEQ_LEN
        ] = s2_data_norm[6]
        eo_style_mask[
            list(params.CHANNEL_GROUPS["S2_NIR_10"]["idx"]), : params.P_MAX_SEQ_LEN
        ] = 0
        # insert NIR 20
        eo_style_array[
            list(params.CHANNEL_GROUPS["S2_NIR_20"]["idx"]), : params.P_MAX_SEQ_LEN
        ] = s2_data_norm[7]
        eo_style_mask[
            list(params.CHANNEL_GROUPS["S2_NIR_20"]["idx"]), : params.P_MAX_SEQ_LEN
        ] = 0
        ...
        # insert Sentinel-1
        eo_style_array[
            list(params.CHANNEL_GROUPS["S1"]["idx"]), : params.P_MAX_SEQ_LEN
        ] = s1a_data_norm[:2]
        # update eo mask
        eo_style_mask[
            list(params.CHANNEL_GROUPS["S1"]["idx"]), : params.P_MAX_SEQ_LEN
...
def init_class_weights_pastis():
    ...
    label_count = torch.zeros(
        (params.P_NUM_OUTPUTS), dtype=torch.long, device=params.DEVICE
    )
    total_count = 0
    ...
    with torch.no_grad():
        for i, image in enumerate(train_dl):
            ...
            label_count += torch.bincount(
                labeled_region, minlength=params.P_NUM_OUTPUTS
            )
