...
class PreTrainingDataset():
    ...
    def generate_input(self, dataset, vis=False):
        ...
        data_list=['s1_s2_era5_srtm_2020_2021.npy', 'dynamicworldmonthly2020_2021.npy', 'latlon.npy']

        for i,sample in enumerate(dataset):
            # get satellite data -> check if no data is available -> skip sample
            if all(key not in sample for key in data_list):
                print(f'\rDataloading: s1_s2_era5_srtm_2020_2021.npy not available in .tar, sample {i} skipped.{params.EOL_SPACE}', end='')
                continue
            ...
            if ('s1_s2_era5_srtm_2020_2021.npy' in sample):
                ...
                if (eo_num_pixels==params.NUM_PIXELS):
                    ...
                    # normalization
                    eo_data = ((eo_data + params.BANDS_SHIFT)/params.BANDS_DIV).astype(np.float32)
                ...
                # too few pixels
                    ...
                    eo_data=np.zeros((params.IMG_WIDTH*params.IMG_WIDTH,params.MAX_SEQ_LEN,self.eo_num_channels), dtype=np.float32)
                    # initiate as masked
            ...
            if ('latlon.npy' in sample):
                ...
                # too few pixels
                    ...
                    lat_lon=np.zeros((params.IMG_WIDTH*params.IMG_WIDTH,2), dtype=np.float32)
            ...
            if ('dynamicworldmonthly2020_2021.npy' in sample):
                ...
                # too few pixels
                    ...
                    dw_data=np.zeros((params.IMG_WIDTH*params.IMG_WIDTH,params.MAX_SEQ_LEN), dtype=np.float32)
                    # initiate as masked
            ...
            for pix_id in range(params.NUM_PIXELS):
                ...
                eo_mask_msk, dw_mask_msk, eo_data_msk, eo_label_msk, dw_data_msk, dw_label_msk, strategy = self.masking.mask_input(eo_data_tr, dw_data_tr)
