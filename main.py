...
def run_training(
    ...
    assert mode in params.MODES, f'Mode {mode} not implemented. Available modes are: {params.MODES}'

    # define correct model class
    model_class = vistos_att_model if model_type =='att' else vistos_conv_model
    # path to pre-trained model
    pretrained_model_path=os.path.join(params.OUTPUT, f'{model_type}_model_weights_vf{vis_field_size}.pth')
    ...
    if mode=='pretrain':
        ...
        pre_training.init_pretraining(model, checkpointing=params.CHECKPOINT, model_type=model_type)
...
def main(args):
    ...
    'cdds' is the Cameroon Deforestation Drivers Segmentation (CDDS) dataset.
    Pass arguments in the form:

    model architecture: 'conv'/ 'att', training type 'finetune/eval', and 
    dataset: 'pastis'/'cdds', visual field size: '1' (only for att)/'3', '5', 7 
    ...
    # finetuning 
        ...
        # start finetuning
        run_training(mode='finetune', dataset=dataset, vis_field_size=field_size, model_type=model_type)
    # evaluation
    elif args[1]=='eval':
        dataset=args[2]
        field_size=int(args[3])
        # start evaluation
        run_training(mode='eval', dataset=dataset, vis_field_size=field_size, model_type=model_type)
...
if __name__ == "__main__":
    ...
    datasets=['pastis', 'cdds']
    # visual field sizes
    field_sizes=params.VIS_FIELDS
    usage= 'Usage: $ python main.py ([att] ([finetune]|[eval] [pastis]|[cdds] [1|3|5]) | ([pretrain] [1|3|5])) | ([conv] ([finetune]|[eval] [pastis]|[cdds] [1|3|5|7]) | ([pretrain] [1|3|5|7]))'
    ...
    if passed_args[1]=='finetune':
        if len(passed_args)!=4 or passed_args[2] not in datasets or int(passed_args[3]) not in field_sizes:
            print(usage)
            sys.exit(1)
