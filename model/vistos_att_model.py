...
class Encoder(vistos_base.EncoderBase, nn.Module):
    ...
    def __init__(
        ...
        # row tensor (flat)
        self.vis_field_rows = (
            torch.arange(self.vis_field_size)
            .repeat_interleave(self.vis_field_size)
            .to(params.DEVICE)
        )
        # column tensor (flat)
        self.vis_field_cols = (
            torch.arange(self.vis_field_size)
            .repeat(self.vis_field_size)
            .to(params.DEVICE)
        )
    ...
    def forward(
        ...
        for channel_group, data in params.CHANNEL_GROUPS.items():
            ...
            channel_embedding = self.channel_embed(
                torch.tensor(self.band_group_to_idx[channel_group])
                .long()
                .to(params.DEVICE)
            )
        ...
        upd_mask = torch.cat(
            (torch.zeros(x.shape[0])[:, None].to(params.DEVICE), upd_mask), dim=1
        )
        orig_indices = torch.cat(
            (
                torch.zeros(x.shape[0])[:, None].to(params.DEVICE).int(),
                orig_indices + 1,
            ),
            dim=1,
        )
        ...
        pix_pos_embed = (
            torch.cat(
                [
                    self.pix_row_embed(self.vis_field_rows),
                    self.pix_col_embed(self.vis_field_cols),
                ],
                dim=-1,
            )
            .unsqueeze(0)
        ...
        if eval_task:
            ...
            x = (
                self.merge_time_2(x)
                .squeeze(-1)
                .reshape(b, self.vis_field_size, self.vis_field_size, e)
            )
            x = self.norm(x)
            # (b,v,v,e)->(b,e,v,v)
...
class Decoder(vistos_base.DecoderBase, nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv2d(
            encoder_embed_dim,
            encoder_embed_dim,
            kernel_size=(vis_field_size, vis_field_size),
            stride=1,
        )
    ...
    def forward(self, x, orig_indices, x_mask, month):
        ...
        if self.vis_field_size != 1:
            ...
            x = x.reshape(
                b * t_new, self.vis_field_size, self.vis_field_size, e
            ).permute(0, 3, 1, 2)
...
class VistosTimeSeriesSeq2Seq(nn.Module):
    ...
    def construct_finetuning_model(
        ...
        model = VistosFinetuningSimple(self.encoder, head1, img_width=img_width).to(
            params.DEVICE
        )
        model.train()
        return model
...
class VistosFinetuningSimple(VistosFineTuningModel):
    ...
    def __init__(
        self, encoder=Encoder, head=SimpleFinetuningHead, img_width=params.P_IMG_WIDTH
    ):
        super().__init__()
