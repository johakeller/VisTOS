...
BATCH_SIZE = (
    5000  # mini-batch size in samples (pixel time series) only mutliples of 625
)
...
DECODER_MLP_RATIO = (
    2  # multilayer perceptron ratio for decoder (factor for hidden dimension)
)
...
CHANNEL_EMBED_RATIO = (
    0.25  # proportion of channel embedding in added positional embedding (<=1.0)
)
MONTHS_EMBED_RATIO = (
    0.25  # proportion of month embedding in added positional embedding (<=1.0)
)
...
CHANNEL_GROUPS = {
    ...
    "S2_RGB": {"id": 1, "length": 3, "idx": (2, 3, 4)},
    "S2_Red_Edge": {"id": 2, "length": 3, "idx": (5, 6, 7)},
    "S2_NIR_10": {"id": 3, "length": 1, "idx": (8,)},
    "S2_NIR_20": {"id": 4, "length": 1, "idx": (9,)},
    "S2_SWIR": {"id": 5, "length": 2, "idx": (10, 11)},
    "NDVI": {"id": 6, "length": 1, "idx": (16,)},
    "ERA5": {"id": 7, "length": 2, "idx": (12, 13)},
    "SRTM": {"id": 8, "length": 2, "idx": (14, 15)},
}

####################################################################### FINE-TUNING: GENERAL HYPERPARAMETERS ################################################################################################

FT_NUM_TRAIN_SAMPLES = (
    None  # length of the dataloader dataset in samples (pixels time series)
)
FT_NUM_VAL_SAMPLES = (
    None  # length of the dataloader dataset in samples (pixels time series)
)
...
P_S1A_PATH = os.path.join(
    P_PATH, "DATA_S1A"
)  # path to PASTIS-R Sentinel-1 Ascending data
P_S1D_PATH = os.path.join(
    P_PATH, "DATA_S1D"
)  # path to PASTIS-R Sentinel-1 Descending data
...
P_TVERSKY_CLASSES = list(
    range(0, 19)
)  # classes to not ignore for FTl, ignores only 19 -> index 20
...
P_LABELS_INV = {value: key for key, value in P_LABELS.items()}
...
MTCC_BATCH_SIZE = 1792  # number of samples (pixels time series) -> should be more than vis_field_size*FT_IMG_WITDH (otherwise too much padding)
...
MTCC_CHANNEL_GROUPS = {"S2_RGB": [0, 1, 2], "S2_NIR_20": [3], "S2_SWIR": [4, 5]}
...
# to address label name via value
MTCC_LABELS_INV = {value: key for key, value in MTCC_LABELS.items()}
