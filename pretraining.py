...
class PreTraining:
    ...
    def init_pretraining(
        ...
        # init logger
        self.logger = utils.init_logger(name=f'pretraining_{model_type}_vf{self.vis_field_size}')
        ...
        # set optimizer
        optimizer = optim.AdamW(param_groups, lr=self.max_learning_rate, betas=(0.9, 0.95),eps=1e-6)
        ...
        if checkpointing:
            model, optimizer, epoch, iteration = self.load_checkpoint(model, optimizer, epoch, iteration)
        ...
        # create datasets
        train_dataset=self.load_dataset(shuffle_on_load=True, masking_strategies=masking_strategies, url=params.TRAIN_URL, data_length=params.TRAIN_DATA_LENGTH)
        val_dataset=self.load_dataset(shuffle_on_load=False, masking_strategies=masking_strategies, url=params.VAL_URL, data_length=params.VAL_DATA_LENGTH)
        print(f'\rCreating train dataloader ...{params.EOL_SPACE}', end='')
        # training dataloader
        self.train_dataloader = wds.WebLoader(train_dataset, batch_size=params.BATCH_SIZE)
        # validation dataloader
        print(f'\rCreating validation dataloader ...{params.EOL_SPACE}', end='')
        self.validation_dataloader = wds.WebLoader(val_dataset, batch_size=params.BATCH_SIZE)
        
        # start training
        model = self.training(model, optimizer, start_epoch=epoch, start_iteration=iteration, dataloader_len=params.TRAIN_DL_LENGTH)
        # start validation
        ...
        if save:
            ...
            file_name= os.path.join(params.OUTPUT, f'{self.model_type}_model_weights_vf{self.vis_field_size}.pth')
    ...
    def load_checkpoint(
        ...
        try:
            ...
            old_checkpoints=[file for file in os.listdir(params.CACHE) if f'{self.model_type}_model_pt_{self.vis_field_size}' in file]
            ...
            model_checkpoint=torch.load(os.path.join(params.CACHE, old_checkpoint), weights_only=True)
            # update model
    ...
    def update_checkpoint(
        ...
        # save new checkpoint
        model_path=os.path.join(params.CACHE, f'{self.model_type}_model_pt_{self.vis_field_size}_{datetime.now().strftime("%m_%d_%H")}.pth')
    ...
    def training(
        ...
        print(f'\rTraining, dataloader length: {dataloader_len} batches of size {params.BATCH_SIZE}')
        ...
        with tqdm(range(start_epoch,self.epochs), desc='Epoch') as tqdm_epoch:
            ...
            for epoch in tqdm_epoch:
                ...
                for iteration, input_dict in enumerate(tqdm(self.train_dataloader, desc='Training',leave=True)):
                    ...
                    with torch.autocast(device_type=params.DEVICE.type, enabled=torch.cuda.is_available()):
                        ...
                        loss, num_eo_masked, num_dw_masked = self.compound_loss(eo_output[eo_mask], dw_output[dw_mask], eo_loss, dw_loss)                    
                    # backpropagation
                ...
                avg_eo_loss = total_eo_loss/max(total_val_num_eo_values_masked, 1) # avg. loss per masked EO element
                avg_dw_loss = total_dw_loss/max(total_val_num_dw_values_masked, 1) # avg. loss per masked DW element
    ...
    def validation(self, model, dataloader_len=0):
        ...
        print(f'\rValidation, dataloader length: {dataloader_len} batches of size {params.BATCH_SIZE}')
        ...
        with torch.no_grad():
            ...
            for input_dict in tqdm(self.validation_dataloader, desc='Validation',leave=True):
                # fetch data from input dictionary            
                eo_label = input_dict['EO_label']
                eo_mask = input_dict['EO_mask'].bool() 
                dw_label = input_dict['DW_label'].long()
